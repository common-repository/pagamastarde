...
class WcPaylaterGateway extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->settings['ok_url'] = ($this->extraConfig['PMT_URL_OK']!='')?$this->extraConfig['PMT_URL_OK']:$this->generateOkUrl();
        $this->settings['ko_url'] = ($this->extraConfig['PMT_URL_KO']!='')?$this->extraConfig['PMT_URL_KO']:$this->generateKoUrl();
...
EOD;
            ...
            $this->settings['enabled'] = 'no';
        } elseif ($this->extraConfig['PMT_SIMULATOR_MAX_INSTALLMENTS']<'2'
                  || $this->extraConfig['PMT_SIMULATOR_MAX_INSTALLMENTS']>'12') {
            $error_string = __(' solo puede ser pagado de 2 a 12 plazos.', 'paylater');
        } elseif ($this->extraConfig['PMT_SIMULATOR_START_INSTALLMENTS']<'2'
                  || $this->extraConfig['PMT_SIMULATOR_START_INSTALLMENTS']>'12') {
            $error_string = __(' solo puede ser pagado de 2 a 12 plazos.', 'paylater');
        } elseif ($this->extraConfig['PMT_DISPLAY_MIN_AMOUNT']<0) {
    ...
    public function paylaterReceiptPage($order_id)
    {
        try {
            ...
            $orderShoppingCart
                ...
                ->setPromotedAmount(0)
                ->setTotalAmount((string) floor(100 * $order->get_total()))
            ;
            ...
            $orderClient = new Client($this->pmt_public_key, $this->pmt_private_key);
            $pmtOrder = $orderClient->createOrder($orderApiClient);
            if ($pmtOrder instanceof Order) {
            ...
            } elseif ($this->extraConfig['PMT_FORM_DISPLAY_TYPE']=='0') {
                wp_redirect($url);
                exit;
            } else {
                $template_fields = array(
    ...
    public function paylaterNotification()
    {
        try {
            ...
            /** @var WcPaylaterNotify $result */
            $result = $notify->processInformation();
            $wcOrderId = $result->getMerchantOrderId();
        } catch (Exception $exception) {
        ...
        $paymentOrder = new WC_Order($wcOrderId);
    ...
    public function is_available()
    {
        if ($this->enabled==='yes' && $this->pmt_public_key!='' && $this->pmt_private_key!='' &&
            (int)$this->get_order_total()>$this->extraConfig['PMT_DISPLAY_MIN_AMOUNT']) {
            return true;
        }
    ...
    {
        return $this->extraConfig['PMT_TITLE'];
    }
    ...
    public function payment_fields()
    {
        $template_fields = array(
            ...
            'min_installments' => $this->extraConfig['PMT_DISPLAY_MIN_AMOUNT'],
            'message' => $this->extraConfig['PMT_TITLE_EXTRA']
        );
    ...
    private function getExtraConfig()
    {
        ...
        $resultsSelect = $wpdb->get_results("select * from $tableName");
        foreach ($resultsSelect as $result) {
            $key = $result->config;
